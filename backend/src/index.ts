...
// Productionda to'lov maxfiy kalitlari bo'sh bo'lsa, ogohlantirish (soxta webhooklar xavfi).
if (isProd) {
  if (!config.click.secretKey) console.warn("Ogohlantirish: CLICK_SECRET_KEY o'rnatilmagan — Click webhook imzosi tekshirilmaydi.");
  if (!config.payme.merchantKey) console.warn("Ogohlantirish: PAYME_MERCHANT_KEY o'rnatilmagan — Payme webhook auth ochiq qoladi.");
}
