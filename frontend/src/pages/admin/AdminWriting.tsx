...
import { Plus, Pencil, Trash2, ChevronLeft, ChevronRight, Save, X, PenTool } from "lucide-react";
...
export function AdminWriting() {
    ...
    const save = async () => {
        ...
        try {
            ...
            if (editing === "new") await api("/admin/writing", { method: "POST", body });
            else await api(`/admin/writing/${editing}`, { method: "PUT", body });
            setEditing(null);
            await load();
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Talaba shu mavzu asosida arab tilida yozadi. O'rta va Qiyin darajalar.
                    </p>
                </div>
            ...
            <div className="flex gap-2">
                ...
                {DIFFICULTIES.filter(d => d !== "easy").map((d) => (
                    <Button
                        key={d}
            ...
            {editing && (
                <div className="rounded-xl border border-primary/30 bg-card p-5 shadow-md space-y-4">
                    ...
                    <div>
                        ...
                        <textarea
                            dir="rtl"
                            className="mt-1 w-full rounded-lg border border-input bg-background px-4 py-3 text-base min-h-[120px] font-arabic leading-relaxed focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                            placeholder="مثلاً: اُكتُب مَقالَةً عَن أهَمِّيَّةِ التَّعليمِ في حَياةِ الإنسان"
                            value={form.prompt}
                            onChange={(e) => setForm((f) => ({ ...f, prompt: e.target.value }))}
                        />
                        <p className="text-xs text-muted-foreground mt-1">
                            Talaba imtihonda shu mavzuni ko'radi va shu haqda yozadi
                        </p>
                    </div>
